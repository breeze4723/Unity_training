...
public class MeshGenerate : MonoBehaviour
{
    ...
    public void Remove(Vector3[] ReVertiePos)
    {
        if (Input.GetMouseButtonDown(1))
        {

            RePos = Camera.main.ScreenToWorldPoint(new Vector3(Input.mousePosition.x, Input.mousePosition.y, 30.0f));
            Debug.Log(RePos);
            for (int j = 0; j < ReVertiePos.Length; j++)
            {
               
                if (RePos == ReVertiePos[j])
                {
                    Debug.Log(ReVertiePos[j]);
